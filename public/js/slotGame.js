...
class SlotGame extends Phaser.Scene{
    ...
    preload(){
        ...
        slotConfig.symbols.forEach((s)=>{
            if(s.fileNameBlurred != null) {
                if(this.textures.exists(s.name)) this.textures.remove(s.name);        // uncomment for game with multiple slot scenes
                this.load.image(s.name + 'Blurred', "png/Symbols/" + s.fileNameBlurred);
            }
        });
        slotConfig.symbols.forEach((s)=>{
            if(s.animation != null) {
        ...
        this.load.html('submit_form', 'emailVerificationForm.html');
    ...
    create(){   
        ...
        // const modalBackground = this.add.rectangle(0, 0, this.gameWidth, this.gameHeight, 0x000000, 0.5);
        // modalBackground.setOrigin(0);
        // modalBackground.setVisible(true);
        // modalBackground.setDepth(50);

        // const modalContent = this.add.container(this.gameWidth / 2, this.gameHeight / 2);
    ...
    {   
        if(!_loggedIn && _spinCount > 3){
            this.input.enabled = false;
            // console.log("qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq")
        }
        else{
